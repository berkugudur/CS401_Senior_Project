...
from GatingBot import GatingBot
...
def close_gateway():
	game_gateway.close_callback_server()
	game_gateway.close()

# Creating gateway and entrypoint
game_gateway = JavaGateway(gateway_parameters=GatewayParameters(port=4242), callback_server_parameters=CallbackServerParameters());
manager = game_gateway.entry_point

# Create gateway for BotCenter 
bot_center_gateway = JavaGateway(gateway_parameters=GatewayParameters(port=1428))

# Add your AI here, don't forget import. If it is Java AI second parameter must be None
add_ai("PredictAI", GatingBot(game_gateway, bot_center_gateway))
add_ai("Thunder", None)
