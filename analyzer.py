    ...
    for frame_data in round_data:
        ...
        if get_player_values(frame_data["P1"])[0] == "STAND_B" or get_player_values(frame_data["P1"])[0] == "STAND_FB":
            if action != last_action:
                if x_as_category:
                    x_category = convert_dif_x_to_category(get_player_values(frame_data["P1"])[3] - get_player_values(frame_data["P2"])[3])
                else:
                    x_category = get_player_values(frame_data["P1"])[3] - get_player_values(frame_data["P2"])[3]
                writer.writerow([frame_data["current_frame"]] + [get_player_values(frame_data["P1"])[0]] + [get_player_values(frame_data["P2"])[0]] + [x_category] )
        if args["print"] == "1":
            print_frame(frame_data)
...
for round_data in data_["rounds"]:
    write_round_standb(round_data, writer_)
